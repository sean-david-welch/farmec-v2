...
func InitMachines(router *gin.Engine, database *sql.DB, s3Client lib.S3Client, adminMiddleware *middleware.AuthMiddlewareImpl) {
...
func MachineRoutes(router *gin.Engine, handler *handlers.MachineHandler, adminMiddleware *middleware.AuthMiddlewareImpl) {
	...
	protected := machineGroup.Group("").Use(adminMiddleware.AdminRouteMiddleware())
	{
