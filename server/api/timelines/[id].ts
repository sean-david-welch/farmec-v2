...
export const PUT: APIRoute = async ({ request, params }): Promise<Response> => {
  ...
  try {
    ...
    const values = [id, data.title, data.date, data.body];
    ...
    if (result.rows.length > 0) {
      ...
      return new Response(
        JSON.stringify({ error: 'Timeline not found or no changes made' }),
        {
          status: 404,
          headers: { 'Content-Type': 'application/json' },
        }
...
export const DELETE: APIRoute = async ({ params }): Promise<Response> => {
  ...
  try {
    const query = await client.query(
      'DELETE FROM Timeline WHERE id = $1 RETURNING *;',
      [id]
    );

    if (query.rowCount === 0) {
      return new Response(
        JSON.stringify({ message: 'No Timeline found with that ID.' }),
        {
          status: 404,
          headers: { 'Content-Type': 'application/json' },
        }
