...
export const GET: APIRoute = async ({ params }) => {
  ...
  try {
    const query = await client.query(
      'SELECT * FROM "Carousel" WHERE "id" = $1;',
      [id]
    );
...
export const PUT: APIRoute = async ({ request, params }): Promise<Response> => {
  ...
  try {
    ...
    const sql = `UPDATE Carousel SET
    name = $2,
    image = $3,
    WHERE id = $1
    RETURNING *;`;

    const values = [id, data.name, data.image];
    ...
    if (result.rows.length > 0) {
      ...
      return new Response(
        JSON.stringify({ error: 'Carousel not found or no changes made' }),
        {
          status: 404,
          headers: { 'Content-Type': 'application/json' },
        }
...
export const DELETE: APIRoute = async ({ params }): Promise<Response> => {
  ...
  try {
    ...
    if (query.rowCount === 0) {
      return new Response(
        JSON.stringify({ message: 'No Carousel found with that ID.' }),
        {
          status: 404,
          headers: { 'Content-Type': 'application/json' },
        }
